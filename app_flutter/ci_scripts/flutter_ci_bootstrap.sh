...
#
# Los plugins nativos se resuelven con Swift Package Manager en las dos plataformas: no hay
# Podfile ni CocoaPods en ninguna, así que `pod` no hace falta en la imagen de la VM.
#
...
# 2. Forzar Swift Package Manager de forma determinista (no depender del default de Flutter).
flutter config --enable-swift-package-manager

# 3. Resolver dependencias Dart y generar la config de build de Apple SIN compilar:
#    ephemeral/Flutter-Generated.xcconfig + el paquete SPM local.
